...
/*   Updated: 2022/08/06 17:42:05 by snino            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//		envp = ft_strjoin_free("SHLVL=", ft_itoa(ft_atoi(envp + 6)
//				+ 1), 3);
...
int ft_lexer(t_mini *mini)
{
	...
	buff_size = (char *) malloc(ft_strlen(mini->line) + 1);
	buff_size = *ft_split(mini->line, ' ');
	while (buff_size)
	{
...
//			buff_size[i++] = *str++;
//		buff_size[++i] = '\0';
//		ft_lstadd_back(&mini->com_list, ft_lstnew(ft_strdup(buff_size)));
//		str++;
//	}
//	free(buff_size);
	return (0);
